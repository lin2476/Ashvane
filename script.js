...
const formatK = t => t < 1000 ? String(t) : (t / 1000).toFixed(1) + 'k';

const DEFAULT_MODEL = 'deepseek-v4-pro';
const DEFAULT_GM_URL = 'https://generativelanguage.googleapis.com';
const DEFAULT_ASSISTANT_PROMPT = '你是一个高通用性、严谨且富有协作精神的AI助手。你的核心目标不是扮演特定角色，而是动态适配用户的真实需求。';
...
const stripMeta = obj => {
  ...
  const { __savedAt, groups, ...rest } = obj;
  return rest;
};

const normalizeText = v => v == null ? '' : String(v);
...
const svgIco = (n, c) => c
  ? `<img src="https://cdn.jsdelivr.net/npm/@lobehub/icons-static-svg@latest/icons/${n}-color.svg" style="width:1.2em; height:1.2em; vertical-align:middle;">`
...
const DS_MODELS = [
  { id: 'deepseek-v4-pro', name: 'DeepSeek V4 Pro', icC: IC_DS_C, icM: IC_DS_M },
  { id: 'deepseek-v4-flash', name: 'DeepSeek V4 Flash', icC: IC_DS_C, icM: IC_DS_M }
];

const isDeepSeek = id => DS_MODELS.some(m => m.id === id) || (id || '').toLowerCase().includes('deepseek');
const normalizeReasoningEffort = (modelId, effort) => {
  const allow = isDeepSeek(modelId) ? ['off', 'high', 'max'] : ['off', 'low', 'high'];
  return allow.includes(effort) ? effort : 'off';
};
...
const normalizeMessage = (m = {}) => {
  ...
  const msg = {
    ...
    reasoning: normalizeText(m.reasoning)
  };
  if (role === 'assistant') msg.modelId = normalizeText(m.modelId || DEFAULT_MODEL) || DEFAULT_MODEL;
  if (m.startTime != null) {
    const n = Number(m.startTime);
    if (Number.isFinite(n)) msg.startTime = n;
    else delete msg.startTime;
  }
  if (m.genTime != null) msg.genTime = normalizeText(m.genTime);
  else if (m.reasoningTime != null) msg.genTime = normalizeText(m.reasoningTime);
  if (Array.isArray(m.thoughtSignatures) && m.thoughtSignatures.length) {
    msg.thoughtSignatures = m.thoughtSignatures.map(normalizeText).filter(Boolean);
  } else {
    delete msg.thoughtSignatures;
  }
  if (m.isNote) msg.isNote = true;
  else delete msg.isNote;
  return msg;
};
...
const normalizeConversation = (c = {}) => ({
  ...c,
  id: c.id || genId(),
  title: normalizeText(c.title).trim() || '新话题',
  messages: Array.isArray(c.messages) ? c.messages.map(normalizeMessage) : []
});

function fixAsst(a = {}) {
  const modelId = normalizeText(a.modelId || a.geminiModel || DEFAULT_MODEL) || DEFAULT_MODEL;
  const asst = {
    ...a,
    id: a.id || genId(),
    name: normalizeText(a.name).trim() || '未命名助手',
    systemPrompt: normalizeText(a.systemPrompt),
    temperature: normalizeNumber(a.temperature, 1.0),
    topP: normalizeNumber(a.topP, 1.0),
    modelId,
    reasoningEffort: normalizeReasoningEffort(modelId, normalizeText(a.reasoningEffort || 'off')),
    conversations: Array.isArray(a.conversations) ? a.conversations.map(normalizeConversation) : []
  };
  asst.activeConvId = asst.conversations.length && asst.conversations.some(c => c.id === a.activeConvId)
    ? a.activeConvId
    : (asst.conversations[0]?.id || null);
  return asst;
}

function createDefaultAssistant() {
  return fixAsst({ id: 'default-ast', ...DEFAULT_ASSISTANT_TEMPLATE });
}
...
const getModelInfo = id => {
  ...
  return {
    iconColor: '<i class="ph-fill ph-sparkle" style="color:var(--accent); vertical-align:middle;"></i>',
    iconMono: '<i class="ph-fill ph-sparkle" style="vertical-align:middle;"></i>',
    name: id,
    custom: true
  };
};

const getCustomModels = () => [...new Set((state.geminiModels || '').split(',').map(s => s.trim()).filter(Boolean))].filter(id => !DS_MODELS.some(m => m.id === id));
...
let state = {
  ...
  deepseekKey: '',
  geminiKey: '',
  geminiBaseUrl: DEFAULT_GM_URL,
  geminiModels: 'gemini-2.5-pro, gemini-3.0-flash',
  darkMode: false,
  webdavUser: '',
  webdavToken: '',
  __savedAt: 0
...
const saveState = (immediate = false) => {
  ...
  const flush = () => { if (IDB.db) IDB.set(STORE_KEY, state).catch(() => {}); };
  if (immediate) flush();
  else saveTimer = setTimeout(flush, 300);
};

function ensureAssistantSelection() {
  let changed = false;
  if (!Array.isArray(state.assistants)) { state.assistants = []; changed = true; }
  state.assistants.forEach(a => {
    a.reasoningEffort = normalizeReasoningEffort(a.modelId || DEFAULT_MODEL, a.reasoningEffort || 'off');
    if (!Array.isArray(a.conversations)) { a.conversations = []; changed = true; }
    if (a.conversations.length && !a.conversations.some(c => c.id === a.activeConvId)) {
      a.activeConvId = a.conversations[0].id;
      changed = true;
    }
    if (!a.conversations.length && a.activeConvId != null) {
      a.activeConvId = null;
      changed = true;
    }
  });
  if (!state.assistants.length) {
    state.assistants.push(createDefaultAssistant());
    changed = true;
  }
  if (!state.assistants.some(a => a.id === state.activeAstId)) {
...
async function loadState() {
  try {
    const [idb, lsRaw] = await Promise.allSettled([IDB.get(STORE_KEY), Promise.resolve(localStorage.getItem(STORE_KEY))]);
    const idbState = idb.status === 'fulfilled' ? idb.value : null;
    let lsState = null;
    try { lsState = lsRaw.status === 'fulfilled' && lsRaw.value ? JSON.parse(lsRaw.value) : null; } catch (e) {}
    let pick = null;
    if (idbState && lsState) pick = ((idbState.__savedAt || 0) >= (lsState.__savedAt || 0)) ? idbState : lsState;
    else pick = idbState || lsState;
    if (pick) {
      const clean = stripMeta(pick);
      state = { ...state, ...clean };
    }
  } catch (e) {}
  state.assistants = (Array.isArray(state.assistants) ? state.assistants : []).map(fixAsst);
  if (!state.assistants.length) state.assistants.push(createDefaultAssistant());
  if (!state.assistants.some(a => a.id === state.activeAstId)) state.activeAstId = state.assistants[0].id;
  state.assistants.forEach(a => {
    a.reasoningEffort = normalizeReasoningEffort(a.modelId, a.reasoningEffort);
    if (a.conversations.length && !a.conversations.some(c => c.id === a.activeConvId)) a.activeConvId = a.conversations[0].id;
    if (!a.conversations.length) a.activeConvId = null;
  });
  persistLocalState();
  if (IDB.db) IDB.set(STORE_KEY, state).catch(() => {});
}

const DEFAULT_GM_URL_CLEAN = DEFAULT_GM_URL.replace(/\/+$/, '');
...
function showDialog(msg, isInput = false, defaultVal = '') {
  return new Promise(resolve => {
    ...
    const cleanup = () => {
      \$1('dialog-overlay').classList.remove('show');
      off('dialog-ok', 'click', onOk);
      off('dialog-cancel', 'click', onCancel);
      off(inp, 'keydown', onKey);
    };
    ...
    on('dialog-ok', 'click', onOk);
    on('dialog-cancel', 'click', onCancel);
...
function applyTheme() {
  ...
  const ht = \$1('hljs-theme');
  if (ht) ht.href = `https://cdn.jsdelivr.net/npm/highlight.js@11.11.1/styles/github${dark ? '-dark' : ''}.min.css`;
  const themeMeta = \$1('theme-color-meta');
  if (themeMeta) themeMeta.setAttribute('content', dark ? '#000000' : '#f0f0f5');
  if (vditorInstance?.setTheme) {
    try { vditorInstance.setTheme(dark ? 'dark' : 'classic', dark ? 'dark' : 'light'); } catch (e) {}
  }
}
...
function setupPWA() {
  ...
  navigator.serviceWorker.register(swUrl).catch(() => {}).finally(() => setTimeout(() => URL.revokeObjectURL(swUrl), 30000));
}
...
function toggleDrawer(side, forceState) {
  ...
  if (open) app.classList.remove(opp);
}
...
const syncSettingsIfOpen = () => {
  if (\$1('settings-drawer')?.classList.contains('open')) renderSettings();
};

function goToChat(id, fromHistory = false) {
  if (!fromHistory && state.activeAstId === id) {
    closeAll();
    return;
  }
...
function renderAstList() {
  ...
  if (!state.assistants.length) {
    return l.innerHTML = '<div class="empty"><i class="ph ph-ghost empty-icon"></i> 还没有助手，点击右上角 ＋ 创建</div>';
  }
  const activeId = getActiveAst()?.id || state.activeAstId;
  l.innerHTML = state.assistants.map(a => `
    <div class="ast-card ${a.id === activeId ? 'active' : ''}" data-id="${a.id}">
      <div class="ast-name">${esc(a.name)}</div>
      <button class="ast-more" title="更多操作"><i class="ph ph-dots-three-vertical"></i></button>
    </div>
  `).join('');
}
...
function handleAstMore(id, btn) {
  ...
  showDropdown(btn, items, async val => {
    if (val === 'delete' && await showDialog('确定要删除此助手吗？')) {
      ...
      if (!state.assistants.length) state.assistants.push(createDefaultAssistant());
      const next = state.assistants[Math.min(idx, state.assistants.length - 1)] || state.assistants[0];
      if (state.activeAstId === id || !state.assistants.some(a => a.id === state.activeAstId)) state.activeAstId = next?.id || state.assistants[0].id;
      saveState();
      renderAstList();
      renderChatPage();
      syncSettingsIfOpen();
...
function renderChatPage() {
  ...
  if (a.id !== state.activeAstId) {
    state.activeAstId = a.id;
    selectionChanged = true;
  }

  const c = getActiveConv(a);
  if (c && c.id !== a.activeConvId) {
    a.activeConvId = c.id;
    selectionChanged = true;
  }
  ...
  if (\$1('model-chip-btn')) {
    ...
    \$1('model-chip-btn').title = m.name || a.modelId;
  }
  if (\$1('reasoning-btn')) {
    const reasonLabel = { off: '关闭', low: 'Low', high: 'High', max: 'Max' }[a.reasoningEffort] || a.reasoningEffort;
    \$1('reasoning-btn').innerHTML = `<i class="ph ph-brain"></i><span>${reasonLabel}</span>`;
    \$1('reasoning-btn').title = `推理深度：${reasonLabel}`;
  }
...
const md = t => {
  ...
  const text = String(t)
    ...
    .replace(/\\$([\s\S]*?)\\$/g, (_, m) => `$${m}$`);
  try {
    return window.marked ? marked.parse(esc(text)) : esc(text).replace(/\n/g, '<br>');
  } catch (e) {
...
function enhanceCodeBlocks(c) {
  c.querySelectorAll('pre:not(.code-block-wrapper pre)').forEach(pre => {
    ...
    wrapper.innerHTML = `<div class="code-block-header"><span class="code-lang">${lang}</span><div class="code-btns"><button class="code-btn copy-btn" title="复制代码"><i class="ph ph-copy"></i> 复制</button><button class="code-btn fold-btn" title="展开/折叠代码">展开</button></div></div>`;
    ...
    on(copyBtn, 'click', e => {
      ...
      copyText((code || pre).textContent || '').then(() => {
        ...
        setTimeout(() => { btn.innerHTML = '<i class="ph ph-copy"></i> 复制'; }, 1200);
      });
    });
    on(foldBtn, 'click', e => {
      e.stopPropagation();
  ...
  c.querySelectorAll('table:not(.table-wrapper table)').forEach(t => {
...
function makeMsg(msg, idx) {
  ...
  const acts = `
    <button data-a="cp" title="复制"><i class="ph ph-copy"></i></button>
    <button data-a="ed" title="编辑"><i class="ph ph-pencil-simple"></i></button>
    <button data-a="del" title="删除"><i class="ph ph-trash"></i></button>
    <button data-a="re" title="${isAi ? '重新生成' : '重新发送'}"><i class="ph ${isAi ? 'ph-arrows-clockwise' : 'ph-arrow-u-up-left'}"></i></button>
  `;
  d.innerHTML = `
    <div class="bubble">
      ${rHTML}
      <div class="markdown-body">${md(msg.content)}</div>
    </div>
    <div class="msg-actions">
      <div class="actions-left">${isAi ? `<span class="badge">${mInfo.iconMono} ${esc(mInfo.name)}</span><span class="gen-time"><i class="ph ph-timer"></i> ${msg.genTime || (msg.startTime ? ((Date.now()-msg.startTime)/1000).toFixed(0) : '0')}s</span>` : ''}</div>
      <div class="msg-tokens">${isAi ? '<i class="ph ph-tag"></i> ' + formatK(getMsgTokens(msg)) : ''}</div>
      <div class="actions-right">${acts}</div>
    </div>`;
  enhanceCodeBlocks(d);
  return d;
}
...
function renderMessages() {
  ...
  if (!c || !c.messages.length) {
    clearDensityMap();
    return msgs.innerHTML = `<div class="welcome"><div class="emoji"><i class="ph-fill ph-sparkle"></i></div><h3>开始对话</h3><p>输入消息，点击发送按钮开始</p><div class="welcome-hints"><div class="welcome-hint" data-prompt="用简单的语言解释量子计算"><i class="ph ph-microscope"></i> 解释量子计算</div><div class="welcome-hint" data-prompt="写一首关于夏天的中国古诗"><i class="ph ph-pen-nib"></i> 写一首古诗</div><div class="welcome-hint" data-prompt="用Python写一个贪吃蛇游戏"><i class="ph ph-code"></i> 写贪吃蛇游戏</div><div class="welcome-hint" data-prompt="给我一个一周健身计划"><i class="ph ph-barbell"></i> 健身计划</div></div></div>`;
  }
...
function renderTopicList() {
  ...
  if (!a || !a.conversations.length) {
    l.innerHTML = '<div class="empty">暂无话题</div>';
    if (dtk) dtk.textContent = '0k';
    return;
  }

  const activeCid = getActiveConv(a)?.id || a.activeConvId;
  let total = 0;
  l.innerHTML = a.conversations.map(c => {
    const tok = calcConvContextTokens(c, a.systemPrompt);
    total += tok;
    return `<div class="topic-item ${c.id === activeCid ? 'active' : ''}" data-cid="${c.id}">
      <span><i class="ph ph-chat-teardrop-text"></i></span>
      <div class="tinfo">
        <div class="ttitle-wrap">
          <div class="ttitle">${esc(c.title)}</div>
          <span class="nav-tokens">${formatK(tok)}</span>
        </div>
        <div class="tmeta">${c.messages.length} 条消息</div>
      </div>
      <button class="ast-more topic-more" title="更多操作"><i class="ph ph-dots-three-vertical"></i></button>
    </div>`;
  }).join('');
  if (dtk) dtk.textContent = total ? formatK(total) : '0k';
}
...
function renderSettings() {
  ...
  sBody.innerHTML = `
    <div class="section">
      <div class="field">
        <label>助手名称</label>
        <input type="text" id="s-name" value="${esc(a.name)}">
      </div>
      <div class="field">
        <label>系统提示词</label>
        <div class="relative">
          <textarea id="s-prompt" rows="4">${esc(a.systemPrompt)}</textarea>
          <button id="s-prompt-fs-btn" class="icon-btn abs-top-right"><i class="ph ph-corners-out"></i></button>
        </div>
      </div>
      <div class="settings-fold">
        <button class="settings-fold-head"><i class="ph ph-sliders"></i> 高级参数 <i class="ph ph-caret-right arr"></i></button>
        <div class="settings-fold-body">
          <div class="field">
            <label>Temperature：<strong id="s-tval">${a.temperature.toFixed(2)}</strong></label>
            <div class="slider-row">
              <span class="slider-label">0</span>
              <input type="range" id="s-temp" min="0" max="2" step=".05" value="${a.temperature}">
              <span class="slider-label">2</span>
            </div>
          </div>
          <div class="field">
            <label>Top P：<strong id="s-pval">${a.topP.toFixed(2)}</strong></label>
            <div class="slider-row">
              <span class="slider-label">0</span>
              <input type="range" id="s-topp" min="0" max="1" step=".05" value="${a.topP}">
    ...
    <div class="section">
      <div class="settings-fold">
        ...
        <div class="settings-fold-body">
          <div class="field">
            <label>坚果云账号</label>
            <input type="text" id="s-webdavUser" value="${esc(state.webdavUser || '')}" placeholder="坚果云登录邮箱">
          </div>
          <div class="field">
            <label>应用密码</label>
            <input type="password" id="s-webdavToken" value="${esc(state.webdavToken || '')}" placeholder="坚果云的第三方应用密码">
          </div>
          <div class="data-actions">
            <button id="data-push"><i class="ph ph-cloud-arrow-up"></i> 推送</button>
            <button id="data-pull"><i class="ph ph-cloud-arrow-down"></i> 拉取</button>
          </div>
          <div class="hint mt-8">注：已通过 Vercel 服务端代理解决跨域限制。</div>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      <div class="settings-fold">
        <button class="settings-fold-head"><i class="ph ph-plugs"></i> API 密钥与模型 <i class="ph ph-caret-right arr"></i></button>
        <div class="settings-fold-body">
          <div class="field">
            <label>DeepSeek API Key</label>
            <input type="password" id="s-dskey" value="${esc(state.deepseekKey)}">
          </div>
          <div class="field">
            <label>第三方 API Key (Gemini)</label>
            <input type="password" id="s-gmkey" value="${esc(state.geminiKey || '')}">
          </div>
          <div class="field">
            <label>第三方 API 地址</label>
            <input type="text" id="s-gmurl" value="${esc(state.geminiBaseUrl || DEFAULT_GM_URL)}">
          </div>
          <div class="field">
            <label>第三方模型列表</label>
            <input type="text" id="s-gmmodels" value="${esc(state.geminiModels || 'gemini-2.5-pro')}">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      <div class="t-row">
        <span id="theme-label">${state.darkMode ? '<i class="ph-fill ph-moon"></i> 暗色' : '<i class="ph-fill ph-sun"></i> 亮色'}</span>
        <div class="tsw ${state.darkMode ? 'active' : ''}" id="s-theme"></div>
      </div>
    </div>

    <div class="section">
      <div class="data-actions">
        <button id="data-export"><i class="ph ph-upload-simple"></i> 导出备份</button>
        <button id="data-import-btn"><i class="ph ph-download-simple"></i> 导入备份</button>
      </div>
    </div>
...
function updateLive(msgEl, msg) {
  ...
  if (msg.reasoning) {
    ...
    if (msg.content.length > 0 && !rb.dataset.autoClosed) {
  ...
  const tk = msgEl.querySelector('.msg-tokens');
  if (tk) tk.innerHTML = '<i class="ph ph-tag"></i> ' + formatK(getMsgTokens(msg));
}
...
function getApiConfig(a, c) {
  ...
  if (isDeepSeek(a.modelId)) {
    const body = {
      model: a.modelId,
      stream: true,
      messages: [{ role: 'system', content: a.systemPrompt }, ...msgs.map(m => ({ role: m.role, content: m.content }))]
    };
    Object.assign(body, a.reasoningEffort === 'off'
      ? { temperature: a.temperature, top_p: a.topP, thinking: { type: 'disabled' } }
      : { reasoning_effort: a.reasoningEffort, thinking: { type: 'enabled' } });
    return {
    ...
    const contents = msgs.reduce((acc, m) => {
      ...
      if (acc.length && acc[acc.length - 1].role === role) acc[acc.length - 1].parts.push(...parts);
    ...
    const body = {
      ...
      generationConfig: {
        ...
        topP: a.topP,
        thinkingConfig: a.reasoningEffort !== 'off'
          ? {
              includeThoughts: true,
              [a.modelId.includes('gemini-3') ? 'thinkingLevel' : 'thinkingBudget']:
                a.reasoningEffort === 'low'
                  ? (a.modelId.includes('gemini-3') ? 'LOW' : 1024)
                  : (a.modelId.includes('gemini-3') ? 'HIGH' : 8192)
            }
          : undefined
      }
    };
...
async function sendMessage() {
  ...
  const c = ensureConv(a);
  c.messages.push({ role: 'user', content: txt });
  if (c.title === '新话题') {
    const title = txt.replace(/\s+/g, ' ').trim();
    c.title = title.substring(0, 28) + (title.length > 28 ? '…' : '');
  }
  ...
  streaming = true;
  \$1('send-btn')?.classList.add('hidden');
  inputEl.disabled = true;
  \$1('stop-btn')?.classList.remove('hidden');
  const nTk = \$1('chat-nav-tokens');
  if (nTk) nTk.textContent = `(${c.messages.length})`;

  const activeMsgEl = \$1('messages')?.lastElementChild;
  abortCtrl = new AbortController();
  let genTimer = setInterval(() => {
    const el = activeMsgEl?.querySelector('.gen-time');
  ...
  const stickBottom = !userScrolledUp;
  ...
  try {
        ...
        try {
          ...
          if (!isDeepSeek(a.modelId)) {
            for (const p of (chunk.candidates?.[0]?.content?.parts || [])) {
              ...
              if (String(p.thought) === 'true') am.reasoning += (p.text || '');
              else if (p.text) am.content += p.text;
            }
          } else {
    ...
    if (!aborted) am.content = am.content || `❌ 错误：${err.message}`;
  } finally {
    streaming = false;
    abortCtrl = null;
    clearInterval(genTimer);

    if (aborted && !am.content && !am.reasoning) {
      c.messages.pop();
    } else {
      am.genTime = ((Date.now() - am.startTime) / 1000).toFixed(0);
      delete am.startTime;
    }

    saveState(true);
    renderChatPage();
    if (stickBottom) scrollBottom(true, false);
...
async function mergeData(data, promptMsg) {
  ...
  const src = stripMeta(data.data || data);
  let newTopics = 0, updatedTopics = 0, newMessages = 0;

  for (const ia of (src.assistants || [])) {
    ...
    if (ex) {
      for (const ic of fa.conversations) {
        ...
        if (exConv) {
          const sameLen = ic.messages.length === exConv.messages.length;
          const changed = JSON.stringify(ic.messages) !== JSON.stringify(exConv.messages);
          if (ic.messages.length > exConv.messages.length || (sameLen && changed)) {
...
on('import-file', 'change', e => {
  ...
  reader.onload = async ev => {
    try {
      ...
      const st = await mergeData(d.data || d, '导入将深度合并数据，确认吗？');
      if (st) toast(`导入成功：新增 ${st.newTopics} 话题，更新 ${st.updatedTopics} 话题，增补 ${st.newMessages} 消息`, 3500);
    } catch (err) {
      toastText('文件格式错误');
    }
  };
...
function updateScrollThumb() {
  const c = $1('chat-container'), t = $1('density-thumb');
  if (!c || !t || c.scrollHeight <= 0) return;
  const maxScroll = Math.max(c.scrollHeight - c.clientHeight, 1);
  const ratio = Math.min(1, c.clientHeight / c.scrollHeight);
  const h = Math.max(ratio * 100, 2);
  const maxTop = Math.max(0, 100 - h);
  const top = Math.min(maxTop, (c.scrollTop / maxScroll) * maxTop);
  t.style.top = `${top}%`;
  t.style.height = `${h}%`;
}
function updateDensityMap() {
  const map = $1('density-map'), chatC = $1('chat-container'), msgs = \$1('messages');
  if (!map || !chatC || !msgs || chatC.scrollHeight <= 0) {
    clearDensityMap();
    return;
  }
  ...
  dotsC.innerHTML = Array.from(msgs.children).filter(m => m.classList.contains('ai')).map(m => `
    <div class="density-dot" data-idx="${m.dataset.index}" style="top:${(m.offsetTop / chatC.scrollHeight) * 100}%; height:${Math.max((m.offsetHeight / chatC.scrollHeight) * 100, 2)}%;" title="跳至此消息"></div>
  `).join('');
  updateScrollThumb();
}

const chatC = \$1('chat-container');
const chatObserver = 'ResizeObserver' in window ? new ResizeObserver(() => requestAnimationFrame(updateDensityMap)) : null;
if (chatObserver && chatC) chatObserver.observe(chatC);
if (chatObserver && $1('messages')) chatObserver.observe($1('messages'));

let isTouching = false;
const setTouch = v => () => isTouching = v;
if (chatC) {
  ...
  on(chatC, 'scroll', function() {
    ...
    if (dist <= 25) userScrolledUp = false;
    else if (isTouching) userScrolledUp = true;
...
function showDropdown(anchor, items, onSelect) {
  ...
  _ddAnchor = anchor;
  dd.innerHTML = items.map((item, i) => item.isHeader
    ? `<div class="dropdown-header">${esc(item.label)}</div>`
    : `<div class="dropdown-item ${item.selected ? 'selected' : ''}" data-idx="${i}">${item.icon || ''}<span>${esc(item.label)}</span>${item.selected ? '<i class="ph ph-check check"></i>' : ''}</div>`
  ).join('');

  const r = anchor.getBoundingClientRect();
  const mh = dd.offsetHeight;
  ...
  _ddActive = e => {
    ...
    if (item) {
      Promise.resolve(onSelect(items[item.dataset.idx].value)).catch(() => {});
      hideDropdown();
    }
  };
...
function handleTopicMore(cid, btn) {
  ...
  const conv = a.conversations[idx], items = [
    { label: '重命名话题', value: 'rename', icon: '<i class="ph ph-pencil-simple"></i>' },
    { label: '复制话题', value: 'copy', icon: '<i class="ph ph-copy"></i>' },
    ...(idx > 0 ? [{ label: '上移', value: 'up', icon: '<i class="ph ph-arrow-up"></i>' }] : []),
    ...(idx < a.conversations.length - 1 ? [{ label: '下移', value: 'down', icon: '<i class="ph ph-arrow-down"></i>' }] : []),
    { label: '删除话题', value: 'delete', icon: '<i class="ph ph-trash"></i>' }
  ];

  showDropdown(btn, items, async val => {
    if (val === 'rename') {
      const nt = await showDialog('重命名话题', true, conv.title);
      if (nt?.trim() && nt.trim() !== conv.title) {
        conv.title = nt.trim();
        saveState();
        renderChatPage();
        renderTopicList();
      ...
      const nc = JSON.parse(JSON.stringify(conv));
      nc.id = genId();
      nc.title = `${conv.title} 副本`;
      a.conversations.splice(idx + 1, 0, nc);
      saveState();
      renderTopicList();
      toast('已复制话题');
    } else if (val === 'up' || val === 'down') {
      const ti = val === 'up' ? idx - 1 : idx + 1;
      [a.conversations[idx], a.conversations[ti]] = [a.conversations[ti], a.conversations[idx]];
      saveState();
      renderTopicList();
...
function openEditModal(msg) {
  editingMsg = msg;
  const ta = $1('edit-textarea');
  const rta = $1('edit-reasoning-textarea');
  const btn = $1('edit-toggle-reasoning-btn');
  const ov = $1('edit-overlay');
  if (!ta || !rta || !btn || !ov) return;

  ta.value = msg.content || '';
  rta.value = msg.reasoning || '';

  const showReasoning = !!msg.reasoning || !!msg.isNote;
  btn.innerHTML = showReasoning ? '<i class="ph ph-note-pencil"></i> 消息备注' : '<i class="ph ph-brain"></i> 思考过程';
  rta.placeholder = showReasoning ? '输入消息备注…' : '输入思考过程…';

  rta.classList.toggle('show', showReasoning);
...
function saveEdit() {
  ...
  const reasoningVisible = $1('edit-reasoning-textarea')?.classList.contains('show');

  if (!val && !rval) return toast('内容不能为空');

  editingMsg.content = val;
  editingMsg.reasoning = rval;

  if (reasoningVisible && !rval) editingMsg.isNote = true;
  else delete editingMsg.isNote;

  saveState();
  renderChatPage();
  $1('edit-overlay')?.classList.remove('show');
  editingMsg = null;
  toast('<i class="ph-fill ph-check-circle"></i> 已保存');
}

// 7. Fullscreen Editor & Events
const syncUndoRedo = () => {
  const ub = $1('fs-undo-btn');
  const rb = $1('fs-redo-btn');
  if (ub) ub.disabled = false;
  if (rb) rb.disabled = false;
};

function setFsPromptMode(rawMode) {
  isFsPromptRawMode = !!rawMode;
  const rawTa = $1('fs-prompt-raw-textarea');
  const vdContainer = $1('fs-prompt-vditor');
  const toggleBtn = $1('fs-prompt-toggle-mode');
  if (!rawTa || !vdContainer || !toggleBtn) return;

  if (isFsPromptRawMode) {
    rawTa.value = vditorInstance ? vditorInstance.getValue() : (rawTa.value || fsPromptOriginalValue);
    vdContainer.classList.add('hidden');
    rawTa.classList.remove('hidden');
    toggleBtn.innerHTML = '<i class="ph ph-markdown-logo"></i>';
    toggleBtn.title = 'Markdown模式';
    rawTa.focus();
  } else {
    if (vditorInstance) vditorInstance.setValue(rawTa.value);
    rawTa.classList.add('hidden');
    vdContainer.classList.remove('hidden');
    toggleBtn.innerHTML = '<i class="ph ph-file-text"></i>';
    toggleBtn.title = '纯文本模式';
  }
  setTimeout(syncUndoRedo, 100);
}

function openFsPromptEditor() {
  const sp = $1('s-prompt');
  const ov = $1('fs-prompt-overlay');
  const rawTa = $1('fs-prompt-raw-textarea');
  const vdContainer = $1('fs-prompt-vditor');
  const toggleBtn = $1('fs-prompt-toggle-mode');
  if (!sp || !ov || !rawTa || !vdContainer || !toggleBtn) return;

  fsPromptOriginalValue = sp.value || '';
  fsPromptChanged = false;
  isFsPromptRawMode = false;

  rawTa.value = fsPromptOriginalValue;
  rawTa.classList.add('hidden');
  vdContainer.classList.remove('hidden');
  ...
  if (!window.Vditor) {
    setFsPromptMode(true);
    toastText('高级编辑器加载中，已使用纯文本模式');
    return;
  }
...
async function handleFsPromptClose(fromPopState = false) {
  const rawTa = $1('fs-prompt-raw-textarea');
  const currentVal = isFsPromptRawMode
    ? (rawTa?.value || '')
    : (vditorInstance ? vditorInstance.getValue() : (rawTa?.value || ''));
...
document.addEventListener('click', async e => {
  ...
  if ((el = get('#ast-drawer-btn'))) toggleDrawer('left');
    ...
    if (!n) return toast('<i class="ph ph-warning-circle"></i> 请输入名称');

    const prompt = $1('new-ast-prompt')?.value.trim() || DEFAULT_ASSISTANT_PROMPT;
    const ast = fixAsst({
      id: genId(),
      name: n,
      systemPrompt: prompt,
    ...
    state.assistants.unshift(ast);
    state.activeAstId = ast.id;
    saveState();
    closeAll();
    renderAstList();
    renderChatPage();
    syncSettingsIfOpen();
    $1('new-ast-name').value = '';
    toast('<i class="ph-fill ph-check-circle"></i> 已创建');
  }
    ...
    if (tg) {
      $1('chat-container')?.scrollTo({ top: tg.offsetTop - 14, behavior: 'smooth' });
      userScrolledUp = true;
    }
  }
  else if ((el = get('#messages'))) {
    const btn = get('button[data-a]');
    const hint = get('.welcome-hint');
    const rhead = get('.rhead');

    if (hint) {
      const uinp = $1('user-input');
      if (uinp) uinp.value = hint.dataset.prompt;
      return sendMessage();
    }

    if (rhead) {
      rhead.classList.toggle('open');
      rhead.nextElementSibling?.classList.toggle('open');
      return;
    }
    ...
    if (btn) {
      ...
      if (act === 'ed') openEditModal(msg);
      else if (act === 'del' && await showDialog('确定要删除这条消息吗？')) {
        ...
        if (msg.role === 'assistant') {
          while (tIdx >= 0 && conv.messages[tIdx].role !== 'user') tIdx--;
          if (tIdx < 0) return toast('无法重新生成：缺用户消息');
        }
        const uinp = $1('user-input');
        if (uinp) {
          uinp.value = conv.messages[tIdx].content;
          uinp.dispatchEvent(new Event('input'));
        }
        conv.messages = conv.messages.slice(0, tIdx);
        saveState();
        renderChatPage();
        syncSettingsIfOpen();
        sendMessage();
    ...
    if (btn('#data-export')) {
      ...
      const blob = new Blob([JSON.stringify({ _meta: { version: 10, date: new Date().toISOString() }, data: snapshotState() }, null, 2)], { type: 'application/json' });
      ...
      if (!auth) return;
      toast('<i class="ph ph-hourglass"></i> 正在同步...');

      const name = getBackupName();
      const data = { _meta: { version: 10, date: new Date().toISOString() }, data: snapshotState() };

      fetchDAV(name, auth, 'PUT', data).then(async res => {
        if (!(res.ok || res.status === 201 || res.status === 204)) {
          throw new Error(`${res.status} [${(await res.text()).toLowerCase().includes('vercel') ? 'Vercel代理未生效' : '被拒绝'}]`);
        }
        let idx = [];
        try {
          const r = await fetchDAV('backup_index.json', auth);
          if (r.ok) idx = await r.json();
        } catch (e) {}
        if (!Array.isArray(idx)) idx = [];
        if (!idx.includes(name)) {
          idx.unshift(name);
          await fetchDAV('backup_index.json', auth, 'PUT', idx);
        }
      ...
      const auth = getWebDAVAuth();
      const pullBtn = btn('#data-pull');
      if (!auth) return;
      toast('<i class="ph ph-hourglass"></i> 正在获取列表...');

      fetchDAV('backup_index.json', auth, 'GET', null, { 'Cache-Control': 'no-cache, no-store, must-revalidate' })
        .then(async res => {
          if (!res.ok) throw new Error(`${res.status} [${res.status === 404 ? '暂无索引，请先推送' : '获取失败'}]`);
          return res.json();
        })
        .then(async files => {
          if (!Array.isArray(files) || !files.length) throw new Error('云端备份列表为空');
          toast('<i class="ph ph-hourglass"></i> 校验真实性...');

          const valids = (await Promise.all(files.map(async f => {
            try { return (await fetchDAV(f, auth, 'HEAD', null, { 'Cache-Control': 'no-cache' })).ok ? f : null; }
            catch (e) { return null; }
          }))).filter(Boolean);
          ...
          showDropdown(pullBtn, [
            { isHeader: true, label: '选择拉取的云端备份' },
            ...valids.map(f => ({ label: f, value: f, icon: '<i class="ph ph-file-json"></i>' }))
          ], async val => {
            ...
            try {
              ...
              toast(`<i class="ph-fill ph-warning-circle"></i> 解析失败: ${err.message}`);
            }
          });
        })
      ...
      const a = getActiveAst();
      const val = id => $1(id)?.value;
      if (!a) return;

      a.name = val('s-name')?.trim() || a.name;
      a.systemPrompt = val('s-prompt')?.trim() ?? a.systemPrompt;

      const temp = parseFloat(val('s-temp'));
      const topp = parseFloat(val('s-topp'));
      if (Number.isFinite(temp)) a.temperature = temp;
      if (Number.isFinite(topp)) a.topP = topp;

      ['dskey:deepseekKey', 'gmkey:geminiKey', 'gmurl:geminiBaseUrl', 'webdavUser:webdavUser', 'webdavToken:webdavToken'].forEach(k => {
        const [id, sk] = k.split(':');
        state[sk] = val('s-' + id)?.trim() ?? state[sk];
      });

      state.geminiModels = val('s-gmmodels')?.trim() || 'gemini-2.5-pro';
      state.darkMode = $1('s-theme')?.classList.contains('active') || false;
      ...
      const head = btn('.settings-fold-head');
      if (head) head.parentElement.classList.toggle('open');

      const themeSw = btn('#s-theme');
      if (themeSw) {
        themeSw.classList.toggle('active');
        const tl = $1('theme-label');
        if (tl) tl.innerHTML = themeSw.classList.contains('active') ? '<i class="ph-fill ph-moon"></i> 暗色' : '<i class="ph-fill ph-sun"></i> 亮色';
      }
    }
  }
  else if ((el = get('#fs-prompt-toggle-mode'))) {
    const rawTa = $1('fs-prompt-raw-textarea');
    const vdContainer = $1('fs-prompt-vditor');
    if (!rawTa || !vdContainer) return;
    setFsPromptMode(!isFsPromptRawMode);
    return;
  }
    ...
    if (sp) {
      ...
      sp.value = newVal;
      fsPromptOriginalValue = newVal;
      fsPromptChanged = false;
      $1('s-save')?.click();
    }
  }
  else if ((el = get('#fs-prompt-close'))) {
    handleFsPromptClose();
  }
  else if ((el = get('#edit-cancel-btn'))) {
    $1('edit-overlay')?.classList.remove('show');
    editingMsg = null;
  }
  else if ((el = get('#edit-save-btn'))) {
    saveEdit();
  }
  else if ((el = get('#edit-toggle-reasoning-btn'))) {
    const rta = $1('edit-reasoning-textarea');
    if (rta) {
      const isActive = rta.classList.toggle('show');
      el.classList.toggle('active', isActive);
      rta.placeholder = isActive ? '输入思考过程…' : '输入思考过程…';
