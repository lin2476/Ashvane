...
const getBackupName = () => { 
  const d = new Date(); 
  return `AIChatBackup-${d.getFullYear()}-${padZero(d.getMonth()+1)}${padZero(d.getDate())}-${padZero(d.getHours())}${padZero(d.getMinutes())}.json`; 
};
...
const saveState = () => { 
  ...
  saveTimer = setTimeout(() => IDB.set(STORAGE_KEY, state).catch(() => toast('存储失败')), 300); 
};
...
function handleAstMore(id, btn) {
  ...
  if (idx < state.assistants.length - 1) items.push({ label: '下移', value: 'order_down', icon: '<i class="ph ph-arrow-down"></i>' });
  ...
  showDropdown(btn, items, async val => {
    if (val === 'delete' && await showDialog('确定要删除此助手吗？')) { 
      ...
      const tgIdx = val === 'order_up' ? idx - 1 : idx + 1; 
      [state.assistants[idx], state.assistants[tgIdx]] = [state.assistants[tgIdx], state.assistants[idx]]; 
    } 
    saveState(); 
    renderAstList();
  });
...
function renderChatPage() {
  ...
  const navTokensEl = $1('chat-nav-tokens'); 
  if (navTokensEl) navTokensEl.textContent = `(${c ? c.messages.length : 0})`;
  
  const modelChipBtn = $1('model-chip-btn'); 
  if (modelChipBtn) modelChipBtn.innerHTML = m.iconColor;
  
  const reasoningBtn = $1('reasoning-btn'); 
  if (reasoningBtn) reasoningBtn.innerHTML = `<i class="ph ph-brain"></i><span>${{ off: '关闭', low: 'Low', high: 'High', max: 'Max' }[a.reasoningEffort] || a.reasoningEffort}</span>`;
  ...
  d.innerHTML = `
    ...
    <div class="msg-actions">
      <div class="actions-left">
        ${isAi ? `<span class="badge">${mInfo.iconMono} ${esc(mInfo.name)}</span><span class="gen-time"><i class="ph ph-timer"></i> ${msg.genTime || (msg.startTime ? ((Date.now()-msg.startTime)/1000).toFixed(0) : '0')}s</span>` : ''}
      </div>
...
function renderTopicList() {
  ...
  const l = $1('topic-list');
  const dtk = $1('drawer-total-tokens'); 
  if(!l) return;
...
function getApiConfig(a, c) {
  ...
  if (isDeepSeek(a.modelId)) {
    ...
    if (/^(sk-|Bearer )/.test(state.geminiKey)) {
      headers['Authorization'] = state.geminiKey.startsWith('Bearer ') ? state.geminiKey : `Bearer ${state.geminiKey}`;
    }
    const contents = msgs.reduce((acc, m) => {
      const parts = [...(m.thoughtSignatures || []).map(s => ({ thoughtSignature: s })), { text: m.content || ' ' }];
      if (acc.length && acc[acc.length - 1].role === (m.role === 'user' ? 'user' : 'model')) {
        acc[acc.length - 1].parts.push(...parts); 
      } else {
        acc.push({ role: m.role === 'user' ? 'user' : 'model', parts });
      }
      return acc;
    }, []);
    const body = { 
      ...
      generationConfig: { 
        ...
        topP: a.topP, 
        thinkingConfig: a.reasoningEffort !== 'off' ? { includeThoughts: true, [a.modelId.includes('gemini-3') ? 'thinkingLevel' : 'thinkingBudget']: a.reasoningEffort === 'low' ? (a.modelId.includes('gemini-3') ? 'LOW' : 1024) : (a.modelId.includes('gemini-3') ? 'HIGH' : 8192) } : undefined 
      } 
    };
...
async function sendMessage() {
  ...
  if (isDeepSeek(a.modelId) ? !state.deepseekKey : !state.geminiKey) {
    return toast('请先设置 API Key') || $1('settings-btn')?.click();
  }
...
function updateDensityMap() {
  ...
  if (!dotsC) { map.innerHTML = '<div id="density-dots"></div><div id="density-thumb"></div>'; dotsC = $1('density-dots'); }
...
const scrollBottom = (force, smooth = false) => { 
  if (force || (!userScrolledUp && !isTouching)) requestAnimationFrame(() => chatC?.scrollTo({ top: chatC.scrollHeight, behavior: smooth ? 'smooth' : 'auto' })); 
};
...
function handleTopicMore(cid, btn) {
  ...
  const items = [
    ...
    ...(idx > 0 ? [{ label: '上移', value: 'order_up', icon: '<i class="ph ph-arrow-up"></i>' }] : []), 
    ...(idx < a.conversations.length - 1 ? [{ label: '下移', value: 'order_down', icon: '<i class="ph ph-arrow-down"></i>' }] : []), 
  ...
  showDropdown(btn, items, async val => {
    if (val === 'rename') { 
      ...
      if (nt?.trim() && nt.trim() !== conv.title) { conv.title = nt.trim(); saveState(); renderChatPage(); renderTopicList(); } 
    }
    else if (val === 'copy') { 
      const nc = JSON.parse(JSON.stringify(conv)); 
      nc.id = genId(); nc.title += ' 副本'; 
      a.conversations.splice(idx + 1, 0, nc); 
      saveState(); renderTopicList(); toast('已复制话题'); 
    }
    else if (val === 'order_up' || val === 'order_down') { 
      const ti = val === 'order_up' ? idx - 1 : idx + 1; 
...
async function handleFsPromptClose(fromPopState = false) {
  ...
  if (currentVal.trim() !== fsPromptOriginalValue.trim()) {
    if (await showDialog('有未保存的修改，是否保存？')) {
      const sp = $1('s-prompt'); 
      if (sp) { sp.value = currentVal; $1('s-save')?.click(); }
    }
  }
...
document.addEventListener('click', async e => {
  ...
  // Ast List Interactions
    ...
    saveState(); closeAll(); renderAstList(); 
  ...
  // Messages Area
    ...
    if (btn) {
      e.stopPropagation(); 
      const act = btn.dataset.a, idx = parseInt(btn.closest('.msg').dataset.index, 10), conv = getActiveConv(), msg = conv.messages[idx];
      ...
      if (act === 'ed') openEditModal(msg);
        ...
        const uinp = $1('user-input'); 
        if(uinp){ uinp.value = conv.messages[tIdx].content; uinp.dispatchEvent(new Event('input')); }
        conv.messages = conv.messages.slice(0, tIdx); 
        saveState(); renderChatPage(); sendMessage();
      }
    }
  }
  ...
  // Settings Panel Interactions
    ...
    if (btn('#data-export')) { 
      ...
      ['dskey:deepseekKey', 'gmkey:geminiKey', 'gmurl:geminiBaseUrl', 'webdavUser:webdavUser', 'webdavToken:webdavToken'].forEach(k => { 
        const[id, sk]=k.split(':'); state[sk] = val('s-'+id)?.trim() ?? state[sk]; 
      });
      state.geminiModels = val('s-gmmodels')?.trim() || 'gemini-2.5-pro'; 
      state.darkMode = $1('s-theme')?.classList.contains('active') || false;
      
      saveState(); applyTheme(); renderChatPage(); renderAstList(); closeAll(); 
      toast('设置已保存');
    } else {
  ...
  // Fullscreen Prompt Custom Toolbar
    ...
    if ((el = get('#fs-prompt-toggle-mode'))) {
      ...
      if (isFsPromptRawMode) { 
        ...
        vdContainer.classList.add('hidden'); rawTa.classList.remove('hidden'); 
        el.innerHTML = '<i class="ph ph-markdown-logo"></i>'; el.title = 'Markdown模式'; 
      } else { 
