...
async function loadState() {
  try {
    ...
    if (p) { delete p.groups; state = { ...state, ...p }; await IDB.set(STORAGE_KEY, state); }
  } catch(e) {}
...
function setupPWA() { 
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register(URL.createObjectURL(new Blob([`self.addEventListener('install',e=>self.skipWaiting());self.addEventListener('activate',e=>self.clients.claim());`], { type: 'application/javascript' }))).catch(()=>{}); 
  }
}
...
function renderChatPage() {
  ...
  const asstNameEl = $1('chat-asst-name'); if (asstNameEl) asstNameEl.innerHTML = esc(a.name);
...
function getApiConfig(a, c) {
  ...
  if (isDeepSeek(a.modelId)) {
    ...
    const contents = msgs.reduce((acc, m) => {
      ...
      if (acc.length && acc[acc.length - 1].role === (m.role === 'user' ? 'user' : 'model')) acc[acc.length - 1].parts.push(...parts); 
      else acc.push({ role: m.role === 'user' ? 'user' : 'model', parts });
      return acc;
    }, []);
...
async function sendMessage() {
  ...
  try {
      ...
      for (let t of lines) {
        ...
        try {
          ...
          if (!isDeepSeek(a.modelId)) {
            ...
            if (d?.reasoning_content) am.reasoning += d.reasoning_content; if (d?.content) am.content += d.content; 
          }
    ...
    streaming = false; abortCtrl = null; clearInterval(genTimer); am.genTime = ((Date.now() - am.startTime) / 1000).toFixed(0); delete am.startTime; 
    saveState(); renderChatPage(); $1('send-btn')?.classList.remove('hidden'); inputEl.disabled = false; $1('stop-btn')?.classList.add('hidden'); 
  }
}
...
async function mergeData(data, promptMsg) {
  ...
  renderChatPage(); closeAll(); return { newTopics, updatedTopics, newMessages };
}
...
function openEditModal(msg) { 
  ...
  btn.innerHTML = isNote ? '<i class="ph ph-note-pencil"></i> 消息备注' : '<i class="ph ph-brain"></i> 思考过程'; rta.placeholder = isNote ? '输入消息备注…' : '输入思考过程…';
...
const syncUndoRedo = () => {
  ...
  if (isFsPromptRawMode) { ub.disabled = false; rb.disabled = false; } else {
    ...
    ub.disabled = vu ? vu.classList.contains('vditor-menu--disabled') : true; rb.disabled = vr ? vr.classList.contains('vditor-menu--disabled') : true;
  }
};
...
async function handleFsPromptClose(fromPopState = false) {
  ...
  if (currentVal.trim() !== fsPromptOriginalValue.trim() && await showDialog('有未保存的修改，是否保存？')) { const sp = $1('s-prompt'); if (sp) { sp.value = currentVal; $1('s-save')?.click(); } }
...
document.addEventListener('click', async e => {
  ...
  if ((el = get('#ast-drawer-btn'))) toggleDrawer('left');
    ...
    showDropdown(el, [
      { isHeader: true, label: 'DeepSeek 模型' }, ...DS_MODELS.map(m => ({ label: m.name, value: m.id, selected: a.modelId === m.id, icon: m.iconColor })), 
      { isHeader: true, label: '第三方 API 模型' }, ...getCustomModels().map(n => ({ label: n, value: n, selected: a.modelId === n, icon: getModelInfo(n).iconColor }))
    ], id => {
      a.modelId = id; const v = isDeepSeek(id) ? ['off', 'high', 'max'] : ['off', 'low', 'high']; 
      if (!v.includes(a.reasoningEffort)) a.reasoningEffort = 'off'; saveState(); renderChatPage(); toast('已切换模型');
    });
  }
    ...
    if (btn) {
      ...
      if (act === 'ed') openEditModal(msg);
        ...
        let tIdx = idx; 
        if (msg.role === 'assistant') { while (tIdx >= 0 && conv.messages[tIdx].role !== 'user') tIdx--; if (tIdx < 0) return toast('无法重新生成：缺用户消息'); }
