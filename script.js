...
const getBackupName = () => {
  const d = new Date();
  return `AIChatBackup-${d.getFullYear()}-${pad0(d.getMonth()+1)}${pad0(d.getDate())}-${pad0(d.getHours())}${pad0(d.getMinutes())}.json`;
};
const getTokens = str => {
  if (!str) return 0;
  const zh = (str.match(/[\u4e00-\u9fa5]/g) || []).length;
  return Math.ceil(zh + (str.length - zh) * 0.25);
};
...
let state = {
  assistants: [{
    id: 'default-ast',
    name: '全能助手',
    systemPrompt: '你是一个高通用性、严谨且富有协作精神的AI助手。你的核心目标不是扮演特定角色，而是动态适配用户的真实需求。',
    temperature: 1.0,
    topP: 1.0,
...
function fixAsst(a) {
  return {
    ...
    temperature: a.temperature ?? 1.0,
    topP: a.topP ?? 1.0,
    modelId: a.modelId || a.geminiModel || DEFAULT_MODEL,
...
const saveState = () => {
  ...
  saveTimer = setTimeout(() => IDB.set(STORE_KEY, state).catch(() => toast('存储失败')), 300);
};
...
function applyTheme() {
  ...
  if (ht) ht.href = `https://cdn.jsdelivr.net/npm/highlight.js@11.11.1/styles/github${state.darkMode ? '-dark' : ''}.min.css`;
}

function setupPWA() {
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register(
      URL.createObjectURL(new Blob([`self.addEventListener('install',e=>self.skipWaiting());self.addEventListener('activate',e=>self.clients.claim());`], { type: 'application/javascript' }))
    ).catch(() => {});
  }
}
...
function toggleDrawer(side, forceState) {
  ...
  const cls = isLeft ? 'left-open' : 'right-open';
  const opp = isLeft ? 'right-open' : 'left-open';
...
function renderAstList() {
  ...
  if (!l) return;
  if (!state.assistants.length) {
    l.innerHTML = '<div class="empty"><i class="ph ph-ghost empty-icon"></i> 还没有助手，点击右上角 ＋ 创建</div>';
    return;
  }
  l.innerHTML = state.assistants.map(a => `
    <div class="ast-card ${a.id === state.activeAstId ? 'active' : ''}" data-id="${a.id}">
      ...
      <button class="ast-more" data-more="1"><i class="ph ph-dots-three-vertical"></i></button>
    </div>
  `).join('');
}

function handleAstMore(id, btn) {
  const idx = state.assistants.findIndex(a => a.id === id);
  if (idx < 0) return;
  ...
  c.querySelectorAll('pre:not(.code-block-wrapper pre)').forEach(pre => {
    ...
    on(wrapper.querySelector('.copy-btn'), 'click', e => {
      ...
      copyText((code || pre).textContent || '').then(() => {
        ...
        setTimeout(() => e.target.innerHTML = '<i class="ph ph-copy"></i> 复制', 1200);
      });
    });
    on(wrapper.querySelector('.fold-btn'), 'click', e => {
      e.stopPropagation();
      e.target.textContent = wrapper.classList.toggle('collapsed') ? '展开' : '折叠';
    });
    if (code && window.hljs) {
...
function makeMsg(msg, idx) {
  ...
  const rHTML = msg.reasoning
    ? `<div class="rblock"><button class="rhead"><span>${msg.isNote ? '<i class="ph ph-note-pencil"></i> 消息备注' : '<i class="ph ph-brain"></i> 思考过程'}</span><i class="ph ph-caret-right arr"></i></button><div class="rbody">${esc(msg.reasoning)}</div></div>`
    : '';
...
function renderTopicList() {
  ...
  l.innerHTML = a.conversations.map(c => {
    ...
    total += tok;
    return `<div class="topic-item ${c.id === a.activeConvId ? 'active' : ''}" data-cid="${c.id}"><span><i class="ph ph-chat-teardrop-text"></i></span><div class="tinfo"><div class="ttitle-wrap"><div class="ttitle">${esc(c.title)}</div><span class="nav-tokens">${formatK(tok)}</span></div><div class="tmeta">${c.messages.length} 条消息</div></div><button class="ast-more topic-more" data-more="1"><i class="ph ph-dots-three-vertical"></i></button></div>`;
  }).join('');
  if (dtk) dtk.textContent = formatK(total);
}

function renderSettings() {
  const a = getActiveAst(), sBody = $1('settings-body');
  if (!a || !sBody) return;
  sBody.innerHTML = `
    <div class="section">
      <div class="field"><label>助手名称</label><input type="text" id="s-name" value="${esc(a.name)}"></div>
      <div class="field"><label>系统提示词</label><div class="relative"><textarea id="s-prompt" rows="4">${esc(a.systemPrompt)}</textarea><button id="s-prompt-fs-btn" class="icon-btn abs-top-right"><i class="ph ph-corners-out"></i></button></div></div>
      <div class="settings-fold">
        ...
        <div class="settings-fold-body">
          <div class="field"><label>Temperature：<strong id="s-tval">${a.temperature.toFixed(2)}</strong></label><div class="slider-row"><span class="slider-label">0</span><input type="range" id="s-temp" min="0" max="2" step=".05" value="${a.temperature}"><span class="slider-label">2</span></div></div>
    ...
    <div class="section">
      <div class="settings-fold">
        ...
        <div class="settings-fold-body">
          ...
          <div class="data-actions"><button id="data-push"><i class="ph ph-cloud-arrow-up"></i> 推送</button><button id="data-pull"><i class="ph ph-cloud-arrow-down"></i> 拉取</button></div>
    ...
    <div class="section">
      <div class="settings-fold">
        ...
        <div class="settings-fold-body">
          <div class="field"><label>DeepSeek API Key</label><input type="password" id="s-dskey" value="${esc(state.deepseekKey)}"></div>
          <div class="field"><label>第三方 API Key (Gemini)</label><input type="password" id="s-gmkey" value="${esc(state.geminiKey || '')}"></div>
          <div class="field"><label>第三方 API 地址</label><input type="text" id="s-gmurl" value="${esc(state.geminiBaseUrl || DEFAULT_GM_URL)}"></div>
          <div class="field"><label>第三方模型列表</label><input type="text" id="s-gmmodels" value="${esc(state.geminiModels || 'gemini-2.5-pro')}"></div>
        </div>
      </div>
    </div>
...
function getApiConfig(a, c) {
  ...
  if (isDeepSeek(a.modelId)) {
    ...
    return {
      url: `${(state.geminiBaseUrl || DEFAULT_GM_URL).replace(/\/+$/, '')}/v1beta/models/${a.modelId}:streamGenerateContent?alt=sse`,
      headers,
      body
    };
  }
...
async function sendMessage() {
  ...
  if (!inputEl) return;
  const txt = inputEl.value.trim(), a = getActiveAst();
  if (!txt || !a) return;
  ...
  try {
    ...
    if (err.name !== 'AbortError') am.content = am.content || `❌ 错误：${err.message}`;
  } finally {
    streaming = false;
    abortCtrl = null;
...
const fetchDAV = (p, auth, m = 'GET', b = null, h = {}) =>
  fetch(`/webdav-proxy/AIChat/${p}`, { method: m, headers: { Authorization: auth, ...(b ? { 'Content-Type': 'application/json' } : {}), ...h }, body: b ? JSON.stringify(b) : null });

function updateScrollThumb() {
  const c = $1('chat-container'), t = $1('density-thumb');
  if (!c || !t || c.scrollHeight <= 0) return;
  t.style.top = (c.scrollTop / c.scrollHeight) * 100 + '%';
  t.style.height = Math.max((c.clientHeight / c.scrollHeight) * 100, 2) + '%';
}

function updateDensityMap() {
  const map = $1('density-map'), chatC = $1('chat-container'), msgs = $1('messages');
  if (!map || !chatC || !msgs || chatC.scrollHeight <= 0) return;
  let dotsC = $1('density-dots');
  if (!dotsC) {
    map.innerHTML = '<div id="density-dots"></div><div id="density-thumb"></div>';
    dotsC = $1('density-dots');
  }
  dotsC.innerHTML = Array.from(msgs.children)
    .filter(m => m.classList.contains('ai'))
    .map(m => `<div class="density-dot" data-idx="${m.dataset.index}" style="top:${(m.offsetTop / chatC.scrollHeight) * 100}%; height:${(m.offsetHeight / chatC.scrollHeight) * 100}%;" title="跳至此消息"></div>`)
    .join('');
  updateScrollThumb();
}
...
if (chatC) {
  ...
  on(chatC, 'scroll', function () {
    ...
    const sd = $1('scroll-down');
    if (sd) sd.classList.toggle('show', dist > 200 && $1('messages').children.length > 1);
  });
}
...
function showDropdown(anchor, items, onSelect) {
  ...
  dd.innerHTML = items.map((item, i) =>
    item.isHeader
      ...
      : `<div class="dropdown-item ${item.selected ? 'selected' : ''}" data-idx="${i}">${item.icon || ''}<span>${item.label}</span>${item.selected ? '<i class="ph ph-check check"></i>' : ''}</div>`
  ).join('');

  dd.classList.add('show');
  const r = anchor.getBoundingClientRect();
  const rect = dd.getBoundingClientRect();
  let top = r.top - rect.height - 4 > 0 ? r.top - rect.height - 4 : r.bottom + 4;
  if (top + rect.height > window.innerHeight) top = Math.max(8, window.innerHeight - rect.height - 8);
  let left = Math.max(5, Math.min(r.left, window.innerWidth - rect.width - 5));
  dd.style.cssText = `top:${top}px; left:${left}px`;
...
function handleTopicMore(cid, btn) {
  const a = getActiveAst(), idx = a?.conversations.findIndex(c => c.id === cid);
  if (idx < 0) return;
...
function saveEdit() {
  ...
  if (rval && $1('edit-toggle-reasoning-btn')?.innerHTML.includes('备注')) editingMsg.isNote = true;
...
document.addEventListener('click', async e => {
  ...
  if ((el = get('#ast-drawer-btn'))) toggleDrawer('left');
    ...
    showDropdown(el, rOpts.map(o => ({ label: o.label, value: o.value, selected: a.reasoningEffort === o.value, icon: '<i class="ph ph-brain"></i>' })), val => {
    ...
    state.assistants.unshift({
      id: genId(),
      name: n,
      systemPrompt: $1('new-ast-prompt')?.value.trim(),
      temperature: 1.0,
      topP: 1.0,
      modelId: DEFAULT_MODEL,
      reasoningEffort: 'off',
    ...
    const card = e.target.closest('.ast-card');
    if (!card) return;
    const more = e.target.closest('.ast-more');
    if (more) {
      e.stopPropagation();
      return handleAstMore(card.dataset.id, more);
    }
    goToChat(card.dataset.id);
  }
  else if ((el = get('#topic-list'))) {
    const a = getActiveAst(), item = e.target.closest('.topic-item');
    if (!a || !item) return;
    const more = e.target.closest('.topic-more');
    if (more) return handleTopicMore(item.dataset.cid, more);
    ...
    if (btn('#data-export')) {
      ...
      fetchDAV(name, auth, 'PUT', data).then(async res => {
        ...
        let idx = [];
        try { const r = await fetchDAV('backup_index.json', auth); if (r.ok) idx = await r.json(); } catch (e) {}
      ...
      const auth = getWebDAVAuth(), pullBtn = btn('#data-pull');
      if (!auth) return;
      ...
      fetchDAV('backup_index.json', auth, 'GET', null, { 'Cache-Control': 'no-cache, no-store, must-revalidate' }).then(async res => {
        ...
        const valids = (await Promise.all(files.map(async f => {
          try { return (await fetchDAV(f, auth, 'HEAD', null, { 'Cache-Control': 'no-cache' })).ok ? f : null; } catch (e) { return null; }
        }))).filter(Boolean);
      ...
      const a = getActiveAst(), val = id => $1(id)?.value;
      if (!a) return;
      a.name = val('s-name')?.trim() || a.name;
      a.systemPrompt = val('s-prompt')?.trim() ?? a.systemPrompt;
      a.temperature = parseFloat(val('s-temp') || a.temperature);
      a.topP = parseFloat(val('s-topp') || a.topP);
    ...
    if ((el = get('#fs-prompt-toggle-mode'))) {
      const rawTa = $1('fs-prompt-raw-textarea'), vdContainer = $1('fs-prompt-vditor');
    ...
    if (btn) {
      ...
      if (isFsPromptRawMode) {
        $1('fs-prompt-raw-textarea').focus();
